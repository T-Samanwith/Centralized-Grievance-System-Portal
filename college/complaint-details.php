...
<head>
	...
	<link rel="icon" type="image/png" href="img/logoMECS-ConnectX.png">
	<title>Complaint Details | Grievance Section | MECS-ConnectX</title>
	<!-- Bootstrap Core CSS -->
...
<table cellpadding="0" cellspacing="0" border="0" class="datatable-1 table table-bordered table-striped	table-responsive display" width="100%">
									<tbody>
<?php $st='closed';
$query=mysqli_query($db,"SELECT facultycomplain.*,academy.collegeName AS name,facultycategory.categoryName
 AS catname FROM facultycomplain JOIN academy ON academy.id=complain.userId 
JOIN faculty ON facultycategory.id=complain.grievanceId 
WHERE facultycomplain.complaintNumber='".$_GET['cid']."'");
...
<?php }?>
<?php $ret=mysqli_query($db,"select facultycomplaintremark.remark as remark,facultycomplaintremark.status as sstatus,facultycomplaintremark.remarkDate as rdate from facultycomplaintremark join complain on complain.complaintNumber=facultycomplaintremark.complaintNumber where facultycomplaintremark.complaintNumber='".$_GET['cid']."'");
while($rw=mysqli_fetch_array($ret))
{
?>
<tr>
